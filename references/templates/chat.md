...
🤖 Generated by follow-news v<VERSION> | 🔗 https://github.com/tangwz/follow-news
```
...
- Write `{{summary}}` in `<LANGUAGE>` as one compact paragraph of 100-300 words depending on article length and substance.
...
- Keep item title lines to the index and source title only. Internal ranking fields can still determine ordering and filtering.
...
Chat summaries normally use 100-300 words depending on article length and substance. Platform length limits take precedence over sentence-count targets when the output is sent through constrained channels.
